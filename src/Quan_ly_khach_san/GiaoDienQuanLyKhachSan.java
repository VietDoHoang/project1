...
public class GiaoDienQuanLyKhachSan {
	...
	private void initialize() {
		...
		frame.setBounds(100, 100, 1020, 565);
		...
		frame.getContentPane().add(tenkhachhang);
		...
		frame.getContentPane().add(chungminhthu);
		...
		frame.getContentPane().add(phong);
		phong.setColumns(10);
		...
		frame.getContentPane().add(sobuoi);
			...
			public void actionPerformed(ActionEvent e) {
				String a=JOptionPane.showInputDialog("Điền Chứng Minh Thư").toString();
					if(danhsach.calculator(a)==0) {
						JOptionPane.showMessageDialog(frame, "Chứng minh thư không tồn tại");
					}else {
						JOptionPane.showMessageDialog(frame,"Khách có chứng minh thư là "+a+" cần thanh toán số tiền là " +Integer.toString(danhsach.calculator(a))+"$");
					}
		...
		frame.getContentPane().add(tinhtien);
		...
		// data mặc định
		nguoi nguoi2 = new nguoi("Đỗ Hoàng Việt",21,"022201000666","0615","A",2);
		nguoi nguoi3 = new nguoi("Nguyễn Văn A",22,"022201000777","0616","B",3);
		nguoi nguoi4 = new nguoi("Vũ Trọng Quảng", 22, "123456789", "0212", "C", 1);
		danhsach.add(nguoi2);
		danhsach.add(nguoi3);
		danhsach.add(nguoi4);
		showData();
		...
		themkhachhang.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(tenkhachhang.getText().equals("")||tuoi.getText().equals("")||chungminhthu.getText().equals("")||phong.getText().equals("")||sobuoi.getText().equals("")) {
					JOptionPane.showMessageDialog(frame, "Nhập dữ liệu khách hàng");
				}else {
					...
					danhsach.add(nguoi1);
					JOptionPane.showMessageDialog(frame, "thêm dữ liệu thành công");// thông báo thêm thành công
					showData();
					// clear data 
					clear();
		...
		frame.getContentPane().add(themkhachhang);
		...
		frame.getContentPane().add(xoakhach);
